...
int main(void)
{
    ...
    printf("\r\nAwaiting voice input trigger command (\"Hello Kio !\"):\r\n");
...
void command_exec(uint32_t cmd_id)
{
	switch(cmd_id)
	{
		...
		case 202: // light off
			duty_cycle = 100.f;
			break;
		case 203: // brightness to 50
			duty_cycle = 50.f;
			break;
		case 204: // increase brightness
			duty_cycle = (duty_cycle-25)<=0.f ? 0.f : duty_cycle-25.f;
			break;
		case 205: // decrease brightness
			duty_cycle = (duty_cycle+25)>100.f ? 100 : duty_cycle+25.f;
			break;
		default :
			break;
	};
